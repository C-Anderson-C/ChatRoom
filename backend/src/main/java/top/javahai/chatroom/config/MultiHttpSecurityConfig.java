...
//    VerificationCodeFilter verificationCodeFilter;
        ...
        @Override
        public void configure(WebSecurity web) throws Exception {
            web.ignoring().antMatchers("/login","/verifyCode","/file","/ossFileUpload",
                    "/user/register","/user/checkUsername","/user/checkNickname",
                    "/ws/**", "/app/**", "/topic/**", "/socket/**");  // 添加WebSocket相关路径
        }
            ...
            http.cors().and() // 添加CORS支持
                    ...
                    .successHandler(new AuthenticationSuccessHandler() {
                        ...
                        public void onAuthenticationSuccess(HttpServletRequest req, HttpServletResponse resp, Authentication authentication) throws IOException, ServletException {
                            ...
                            // 修改这里，从ChatUserDetails获取User对象
                            ChatUserDetails userDetails = (ChatUserDetails) authentication.getPrincipal();
                            User user = userDetails.getUser(); // 假设ChatUserDetails有getUser()方法
